...
func (s *CadastralService) DetectConflicts(req dto.DetectConflictRequest, idempotencyKey string, actor Actor) ([]model.TopologyConflict, error) {
	...
	for _, neighbour := range neighbourModels {
		wg.Add(1)
		go func(nm model.LandParcel) {
			defer wg.Done()
			polygon, parseErr := geometry.ParsePolygon(nm.BoundaryGeoJSON)
			if parseErr != nil {
				return
			}
			neighbours = append(neighbours, geometry.ParcelReference{ID: nm.ID, Polygon: polygon})
			references = append(references, polygon)
			hashParts = append(hashParts, strconv.FormatUint(uint64(nm.ID), 10), nm.BoundaryGeoJSON)
		}(neighbour)
	}
	wg.Wait()
