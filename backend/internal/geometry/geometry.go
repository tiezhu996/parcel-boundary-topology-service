...
// polygonCache memoizes parsed boundaries to speed up repeated detection runs.
var polygonCache = make(map[string]Polygon)
...
func ParsePolygon(raw string) (Polygon, error) {
	if cached, ok := polygonCache[raw]; ok {
		return cached, nil
	}
